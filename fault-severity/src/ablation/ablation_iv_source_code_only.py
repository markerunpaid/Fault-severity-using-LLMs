...
"""
ABLATION STUDY — Configuration IV: Source Code Only (Mashhadi et al. RQ2 Style)
================================================================================
Pipeline: Source Code → Fine-tuned LLM → Classification (NO metrics, NO scaler, NO SMOTE)

This is the closest replication of Mashhadi et al.'s RQ2 setup:
  - Only raw source code is fed to the LLM
  - No code metrics are extracted or used
  - No RobustScaler
  - No SMOTE
  - Classifier input = 768-d embedding only: Linear(768, 4)

Data used: data/train_raw.csv + data/test_raw.csv
  (output of step1_preprocess.py — BEFORE step2_extract_metrics.py)

Output: results/ablation/ablation_iv_source_code_only_{model}_results.json
"""
...
# ── Source-code-only model: Linear(768, 4) — NO metrics at all ───────────────
class SourceCodeOnlyModel(nn.Module):
    """
    Encoder + Linear(768, 4).
    Mirrors Mashhadi et al. RQ2: only source code is the input.
    No metrics tensor is used anywhere.
    """
    def __init__(self, model_name: str, num_classes: int = 4, dropout_prob: float = 0.1):
        ...
        print(f"  [SourceCodeOnly] {model_name}")
        print(f"  [SourceCodeOnly] Classifier: Linear({hidden_size}, {num_classes})")
        print(f"  [SourceCodeOnly] No SMOTE | No Scaler | No Metrics")
...
# ── Minimal dataset: source code + label only ────────────────────────────────
class SourceCodeOnlyDataset(Dataset):
    """
    Tokenizes source code only. Returns a dummy zero metrics tensor
    so trainer.py (which calls batch['metrics']) doesn't break.
    The model completely ignores it.
    """
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'metrics':        torch.zeros(10, dtype=torch.float32),  # dummy — ignored
...
# ── Per-model runner ──────────────────────────────────────────────────────────
def run_ablation_iv(model_key):
    cfg    = MODELS[model_key]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"\n{'='*65}")
    print(f"ABLATION IV — Source Code Only (Mashhadi RQ2)  |  {model_key.upper()}")
    print(f"{'='*65}")

    # Load earliest-stage data: only method_code + label (before step2)
    train_df = pd.read_csv("data/train_raw.csv")[['method_code', 'label']]
    test_df  = pd.read_csv("data/test_raw.csv")[['method_code',  'label']]
    print(f"  Train: {len(train_df)}  |  Test: {len(test_df)}")
    print(f"  Input: source code ONLY — no metrics, no scaler, no SMOTE")
    print(f"  Class distribution:\n{train_df['label'].value_counts().sort_index().to_string()}")

    tokenizer    = load_tokenizer(model_key, cfg["model_name"])
    train_loader = DataLoader(
        SourceCodeOnlyDataset(train_df, tokenizer, MAX_LENGTH),
        batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=True)
    test_loader  = DataLoader(
        SourceCodeOnlyDataset(test_df, tokenizer, MAX_LENGTH),
        batch_size=8, shuffle=False, num_workers=NUM_WORKERS, pin_memory=True)

    # SourceCodeOnlyModel: Linear(768, 4) — no metrics branch
    model  = SourceCodeOnlyModel(model_name=cfg["model_name"], dropout_prob=cfg["dropout"]).to(device)
    config = {
    ...
    print(f"\n  Results — Ablation IV — {model_key.upper()}")
    ...
    out  = {"ablation": "iv_source_code_only",
            "description": "Source code ONLY → LLM → Linear(768,4). No metrics, scaler, or SMOTE. Mashhadi et al. RQ2.",
            "model": model_key, "model_name": cfg["model_name"], **metrics, "history": history}
    path = f"results/ablation/ablation_iv_source_code_only_{model_key}_results.json"
...
if __name__ == "__main__":
    ...
    results = {k: run_ablation_iv(k) for k in keys}

    print(f"\n{'='*65}\nABLATION IV SUMMARY — Source Code Only (Mashhadi RQ2)\n{'='*65}")
